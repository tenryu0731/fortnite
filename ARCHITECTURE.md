...
- 交戦: 武器種ごとの交戦距離 (ショットガン 8m / SMG 14m / AR 28m / スナイパー 65m) へ
  詰める・下がる。ストレイフ、上位ボットはジャンプストレイフ。
- 被弾時は壁を建てる (BuildSystem と同じ規則)。回復時、上位ボットは四方を囲ってから飲む。
- 聴覚: 75m (スナイパーは 130m) 以内の銃声へ振り向き、確認しに行く。
- ルート: より良い武器・回復アイテム・未開封チェストへ寄り道する。
- 降下: ボットもバスに乗り、飛行経路に沿って順に降りる (着地まで描画も被弾もしない)。
  スターターピストルのボットはまず漁る。交戦距離は着地から 60 秒かけて広がる。

**エイムモデル (人間らしさ)**: 誤差 = 基礎誤差 (skill・難易度) + 捕捉誤差 (新目標で大きく、
追い続けると 0.55〜1.6 秒で収束) + 追従誤差 (目標の見かけの角速度 × skill 依存の遅れ)。
反応 0.22〜0.65 秒。22m 以遠のフルオートはバースト射撃。
デュエル計測 (強ボット, 15m): 静止目標 63% / ストレイフ目標 33% 命中 (旧: 83% / 83%)。

**難易度**: EASY / NORMAL / HARD。毎試合この範囲から skill を抽選し、誤差にも倍率をかける。

### 6.5b `AimAssist`
タッチ操作の標準的な補助 3 種: 照準が敵に掛かっている間の**減速 (friction)**、動く敵への
**追従 (magnetism)**、ADS 開始時に小円錐内の敵へ**吸着 (snap)**。視線が通る時だけ、
ツルハシと建築中は無効、設定で OFF 可。入力ハブの公開値を書き換えるので全入力デバイスに効く。
...
### 9.4 状況把握 (Awareness)
- **被弾方向**: 攻撃者を指す赤い弧。振り向いても攻撃者を指し続ける。
- **音の可視化**: 80m 以内の銃声・24m 以内の足音をリング上のアイコンで表示
  (スマホはミュートで遊ばれることが多いため、ジャンル標準の機能)。設定で OFF 可。
- ボットはプレイヤー風のハンドル名を持つ (キルフィードが「Bot 7」ではなくなる)。

## 9.5 アートディレクション (Art Direction)

目標は「ジャンルの見た目として読めること」— ハイキーで高彩度、影は黒ではなく
色付き、地平線は淡いシアンのもや、草は黄緑、空には大きな積雲。これは
**色統計の問題**なので、議論ではなく数値で合わせて数値でゲートする。

- `tests/stylemetrics.cjs` がフレームを CIE Lab に変換し、平均/分散/5%・95%
  明度、彩度、Hasler–Süsstrunk colourfulness、空と草のクラス平均色と色相を測る。
- 参考フレーム（ジャンルの実プレイ画面）は著作物なので**リポジトリに入れない**。
  入れるのは導出した数値だけ（`tests/style-targets.json`）。
- `tests/style.cjs` はビジュアル回帰が検証済みのベースラインを測り、目標±許容幅
  から外れたら失敗する。旧ルック（暗く灰色）はこのゲートで 7 指標中 3 つ失敗する。

主な構成要素:

| 要素 | 手法 |
|------|------|
| ライティング | ACES + 露出 1.36、暖色の太陽、明るく色付きの半球光（影を持ち上げる） |
| 空 | グラデーションドーム + 平底の積雲（頂点色で上面白/底面青灰、1 draw call） |
| 空気遠近 | 地平線色のフォグを手前から開始、遠景の丘が青く霞む |
| 木 | 複数の塊を1つのボリュームとして陰影付けする**球面法線**（stylised foliage の定石） |
| 水 | 深度から頂点色を焼く（浅瀬アクア→沖の青）、深度で不透明度も変える |
| キャラクター | 共通の `gen/Humanoid.js`。大きめの頭・V字胴・大きな手袋/ブーツ、丸いプリミティブ |
| ボット | 1 InstancedMesh。部位IDと服装パレットテクスチャで多色化、頂点シェーダで歩行 |

**向きの規約**: キャラクターは **-Z を向く**（yaw 0 の `lookDirection()` と同じ）。
旧モデルは +Z 向きで、体と腕が後ろ向きのまま銃だけを逆回転で前に向けていた。
テスト（体の正面・構えた腕・銃口・ボットの銃がすべて視線方向）で固定済み。
...
| ストリーミング | 地形チャンクのメッシュ生成を頂点予算で分割・再開 | ≤ 1100 頂点/フレーム |

品質プリセット `low / medium / high` は
テクスチャ解像度・影の有無・草密度・描画距離・パーティクル上限を切り替える。
初回起動時に `deviceMemory` / `hardwareConcurrency` / DPR から自動推定。

**地形ストリーミングの分割生成**: 近距離チャンク 1 枚は 65×65 = 4225 頂点で、
高さ・法線・湿度・バイオーム色のサンプリングに数ミリ秒かかる。これを 1
フレームで行うと、LOD 境界をスプリントで横切るたびにフレーム落ちが発生する。
そこで `Terrain` はチャンク生成を**再開可能なジョブ**として保持し、
毎フレーム頂点予算 (`VERTEX_BUDGET_PER_FRAME`) の分だけ行単位で進める。
完成するまで旧ジオメトリを描き続けるため、ポップもヒッチも起きない。
予算は時計ではなくジオメトリ由来なので、同じ手順は常に同じメッシュを生む
(決定論を壊さない)。
...
__GAME.stepSim(n)       // 描画せずシミュレーションのみ n ステップ (ロジック検証用)
__GAME.renderOnly(n)    // シミュレーションを進めずに n フレーム描画
__GAME.setRasterScale(s)    // 描画バッファのみ縮小 (レイアウト/DPR/シーンは不変)
__GAME.profileSystems(true) // システム単位の計測を有効化
__GAME.systemMetrics(n)     // 重い順に { key, ms, max, calls }
```

`setRasterScale` はソフトウェアラスタライザ対策である。フラグメント処理は
SwiftShader が最も苦手とし、かつ実機 GPU が最も得意とする部分なので、
描画バッファだけを縮小しても **シミュレーション・更新グラフ・draw call・
三角形数は実機と同一のまま**、単位時間あたりのサンプル数だけが増える。
パーセンタイルが統計として意味を持つ数のフレームを集めるために使う。
フル解像度の描画は視覚回帰スイートが担保する。

### 11.2 検証シナリオ
スクリーンショット回帰の対象 (いずれも決定論モードで静止):

| name | 内容 |
|------|------|
| `gen_gallery` | 生成テクスチャ/メッシュ/波形の一覧 |
| `terrain_wide` / `terrain_ground` / `terrain_coast` | 俯瞰・地表・海岸線 |
| `poi_town` / `poi_factory` / `poi_tower` / `poi_street` | POI 各種 |
| `player_tps` / `player_ads` | 標準 TPS 構図 / ADS 構図 |
| `build_grid` | 建築ゴースト + 設置済み部材 (全部材種) |
| `fx_combat` | マズルフラッシュ・トレーサ・着弾・デカール |
| `bots_squad` | 距離違いのボット隊列 |
| `storm_edge` | ストーム境界 |
| `match_loot` | チェストと全アイテム種 |
| `hud_full` / `screen_result` | HUD 全要素 / リザルト画面 |
| `touch_landscape` / `touch_portrait` / `touch_build` | タッチ UI レイアウト |

計測専用 (回帰対象外、エンジンは動作したまま):

| name | 内容 |
|------|------|
| `perf_battle` | 最悪ケース。POI 内・ボット 24 体が視界内で交戦・ストーム接近中・地面にルート・自陣の建築 36 枚・HUD/ミニマップ稼働 |

> 何もしないで起動した状態はゲーム中**最も軽いフレーム**であり、
> それを計測しても何も保証しない。性能ゲートは `perf_battle` を
> 発砲入力を押しっぱなしのまま計測した値で行う。
...
| 三角形数 | ≤ 350k |
| タッチテスト | 全項目 pass |
| 計測サンプル数 | 各ラン ≥ 60 フレーム (パーセンタイルが統計として成立する下限) |

CPU / draw の閾値は **最悪ケース (`perf_battle`)** に対して適用する。
静止した俯瞰での計測は参考値にすぎない。
